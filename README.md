...
请只在确认拥有公开传播授权时，将音乐文件提交并发布到 GitHub Pages。仓库默认通过 `.gitignore` 忽略 `assets/music.mp3`，避免误把本地测试音乐上传到公开站点。如果你使用的是已获授权或免版权音乐，可以用 `git add -f assets/music.mp3` 强制加入提交。
